...
double nozzle (double X, double Y) {
  ...
  double
    ...
    x2 = round((FeedThL + 1.0*NozzL)/NozzR/Deltaa)*Deltaa,
...
/* #endif */
    ...
    double x2 = round((FeedThL + 1.0*NozzL)/NozzR/Deltaa)*Deltaa,

    fraction (f, union(-y1 + y + 9.*Deltaa,difference(x2 + 9.*Deltaa - x, sq(rad) - sq(y) - sq(x - (x2 + 9.*Deltaa + sqrt(sq(rad) - 1))))));
    
    //fraction (f, union((1. - y1 - 2.*Deltaa),difference((FeedThL + 1.05*NozzL)/NozzR - x + 2.*Deltaa, sq(rad) - sq(y) - sq(x + 2.*Deltaa - ((FeedThL + 1.05*NozzL)/NozzR + sqrt(sq(rad) - 1))))));
