...
int main (int argc, char * argv[]) {
  ...
  Ec=muup/relaxtime;
  muu = (mus + muup);
  fprintf(ferr,"# Beta = %g De = %g Ec = %g  Re = %g We = %g\n",Beta,De,Ec,Reynolds,Weber);
...
event properties (i++) {
    double mupvv = (1. - Beta)/Reynolds;
    double lambdavv = De*sqrt(rho1/f.sigma);
  foreach () {
    Gpd[] = (f[] > 1.-1e-6 ? mupvv/lambdavv: 0.); 
    lambdad[] = (f[] > 1.-1e-6 ? lambdavv: 0.);
  }
}
/* event properties (i++) { */
/*   foreach() { */
...
event snapshot (t=0.0025; t+=0.0025 ; t <= 2.5) {
  ...
  dump(name, (scalar *){f,u,p,cs,omega,kappa,tau_p,tau_qq});
  ...
  if (t == 0)
    output_field({u,p,omega,cs,tau_p,tau_qq}, fpp, n = 2048, linear = true, box = {{X0,Y0},{X0 + L0,Y0 + FeedThR/NozzR}});
  else
    output_field({u,p,omega,tau_p,tau_qq}, fpp, n = 2048, linear = true, box = {{X0,Y0},{X0 + L0,Y0 + FeedThR/NozzR}});
  fclose(fpp);
}
