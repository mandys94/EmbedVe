...
/* #endif */
    ...
    double Deltaa = L0/(1 << LEVEL);
    double y1 = round(FeedThR/NozzR/Deltaa)*Deltaa;
    double nf=5.;

    fraction (f, union(-y1 + y + nf*Deltaa,difference((FeedThL + 1.05*NozzL)/NozzR + nf*Deltaa - x, sq(rad) - sq(y) - sq(x - ((FeedThL + 1.05*NozzL)/NozzR + nf*Deltaa + sqrt(sq(rad) - 1))))));
...
event snapshot (t=0.0025; t+=0.0025 ; t <= 2.5) {
  ...
  dump(name, (scalar *){f,u,p,cs,kappa,tau_p,tau_qq});
