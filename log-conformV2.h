...
event tracer_advection (i++)
{
  ...
  foreach() {
	...
	Psi.x.x[] = sq(R.x.x)*log(Lambda.x) + sq(R.x.y)*log(Lambda.y);
      ...
      pseudo_t B;
      double OM = 0.;
      // change velocity gradients
      if (fabs(Lambda.x - Lambda.y) <= 1e-20) {
	B.x.y = (center_gradient_x(u.y) + center_gradient_y(u.x))/2.;
	//	B.x.y = (u.y[1,0] - u.y[-1,0] +
	//	 u.x[0,1] - u.x[0,-1])/(4.*Delta); 
	foreach_dimension()
	  B.x.x = center_gradient_x(u.x);
	//	  B.x.x = (u.x[1,0] - u.x[-1,0])/(2.*Delta);
      }
      else {
	pseudo_t M;
	foreach_dimension() {
	  M.x.x = sq(R.x.x)*center_gradient_x(u.x) +
	    sq(R.y.x)*center_gradient_y(u.y) +
	    R.x.x*R.y.x*(center_gradient_y(u.x) + 
			 center_gradient_x(u.y));
	  
	  /* M.x.x = (sq(R.x.x)*(u.x[1] - u.x[-1]) + */
	  /* 	   sq(R.y.x)*(u.y[0,1] - u.y[0,-1]) + */
	  /* 	   R.x.x*R.y.x*(u.x[0,1] - u.x[0,-1] +  */
	  /* 			u.y[1] - u.y[-1]))/(2.*Delta); */
	  M.x.y = R.x.x*R.x.y*center_gradient_x(u.x) + 
	    R.x.y*R.y.x*center_gradient_x(u.y) +
	    R.x.x*R.y.y*center_gradient_y(u.x) +
	    R.y.x*R.y.y*center_gradient_y(u.y);
  ...
  foreach()
    ...
    if (lambda[] == 0.) {
      ...
      foreach_dimension()
	tau_p.x.x[] = 2.*mup[]*center_gradient_x(u.x); // 2*mu*dxu;
      //	tau_p.x.x[] = mup[]*(u.x[1,0] - u.x[-1,0])/Delta; // 2*mu*dxu;
      tau_p.x.y[] = mup[]*(center_gradient_x(u.y) +
			   center_gradient_y(u.x)); // mu*(dxv+dyu)
...
#define neigh_center_gradient_x(a) (fs.x[0,-1] && fs.x[1,-1] ? (a[1,-1] - a[-1,-1])/(2.*Delta) : \
			     fs.x[1] ? (a[1] - a[])/Delta :		    \
			     fs.x[]  ? (a[] - a[-1])/Delta : 0)

#define neigh_center_gradient_y(a) (fs.y[-1] && fs.y[-1,1] ? (a[-1,1] - a[-1,-1])/(2.*Delta) : \
			      fs.y[-1,1] ? (a[-1,1] - a[-1])/Delta :        \
			      fs.y[-1]  ? (a[-1] - a[-1,-1])/Delta : 0)
...
/*   foreach_face() */
/*     if (fm.x[] > 1e-20) { */
/*       double shear = (tau_p.x.y[0,1] + tau_p.x.y[-1,1] - */
...
/* } */
/* Returns \int_S s \dot n dS on embed boundary where s = sx ex + sy ey*/
#if EMBED
double tau_embed_flux (Point point, scalar sx, scalar sy){
  ...
  double valx = embed_interpolate (point, sx, p);
  double valy = embed_interpolate (point, sy, p);
  return (valx*n.x + valy*n.y)*area/Delta;
...
event acceleration (i++)
{
  ...
  foreach_face()
    if (fm.x[] > 1e-20) {
      double shear = (center_gradient_y(tau_p.x.y) + neigh_center_gradient_y(tau_p.x.y))/2.;
      av.x[] += (shear +  (tau_p.x.x[] - tau_p.x.x[-1])/Delta)*alpha.x[]/sq(fm.x[]);
    }
#if AXI
  foreach_face(y)
    if (y > 0.)
#if EMBED
      av.y[] -= embed_face_value_y(point,tau_qq,0)
#else
	(tau_qq[] + tau_qq[0,-1])
#endif
	*alpha.y[]/sq(y)/2.;
