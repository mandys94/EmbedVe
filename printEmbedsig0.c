...
p[embed] = neumann(0.);
pf[embed] = neumann(0.);
f[embed] = x < (FeedThL + NozzL)/NozzR ? 1. : 0.;
...
event init (t = 0) {
  if (!restore (file = "restart")) { 
    ...
    fraction (f, (FeedThL + NozzL)/NozzR - x);
...
event adapt (i++) {
  ...
  adapt_wavelet ({f, u, cs_temp}, (double[]){1e-3,5e-3,5e-3,1e-3}, LEVEL, LEVEL - 4);
...
event snapshot (t += 0.01; t <= 2.5) {
  ...
  dump(name, (scalar *){f,u,p,cs,omega});
  ...
  if (t == 0)
    ...
    output_field({u,p,omega}, fpp, n = 2048, linear = true, box = {{X0,Y0},{X0 + L0,Y0 + FeedThR/NozzR}});
  fclose(fpp);
}
